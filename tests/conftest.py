...
def generate_test_data(num_records):
    ...
    for _ in range(num_records):
        ...
        # Ensure b is not 0 for divide operation to prevent division by 0 in expected calculation
...
def pytest_generate_tests(metafunc):
    ''' Check if the test is expecting
    # any of the dynamically generated fixtures '''
    if {"a", "b", "expected"}.intersection(set(metafunc.fixturenames)):
        num_records = metafunc.config.getoption("num_records")
        '''Adjust the parameterization to include both oper_name and operation for compatibility
